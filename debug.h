...
#if SHOW_MSG
	#define MSG(format, ...) do{PRINTFUNC(PRINT_FD, "\033[01;32mMSG(\e[m\033[01;36mF:%s L:%d\e[m\033[01;33m):\e[m" format, __func__ , __LINE__ , ##__VA_ARGS__); fflush(PRINT_FD); }while(0)
#else
	#define MSG(format, ...)
#endif
#define ERROR(format, ...) do{PRINTFUNC(PRINT_FD, "\033[01;31mERROR\e[m (F:%s L:%d): " format, __func__ , __LINE__ , ##__VA_ARGS__); _nv_error_count++; if(SPIN_ON_ERROR){ _nvp_debug_handoff(); } }while(0)
...
#if PRINT_DEBUG_FILE
#define DEBUG_FILE(format, ...) do {PRINTFUNC(DEBUG_FD, "\033[01;33mDEBUG (\e[m\033[01;35mF:%s L:%d\e[m\033[01;32m):\e[m" format, __func__ ,__LINE__, ##__VA_ARGS__); fflush(PRINT_FD); }while(0)
#else
...
#if SHOW_DEBUG
	#define DEBUG(format, ...)   do{PRINTFUNC(PRINT_FD, "\033[01;33mDEBUG (\e[m\033[01;35mF:%s L:%d\e[m\033[01;32m):\e[m"   format, __func__ , __LINE__ , ##__VA_ARGS__); fflush(PRINT_FD); } while(0)
    #define WARNING(format, ...) do{PRINTFUNC(PRINT_FD, "\033[01;36mWARNING (\e[m\033[01;35mF:%s\e[m\033[01;32m):\e[m " format, __func__ , ##__VA_ARGS__); } while(0)
